...
/*
   ...
   *
   */
export function toFormat(number, dp, rm, fmt) {
  if (!number.e && number.e !== 0) return number.toString(); // Infinity/NaN
  ...
  arr = number.toFixed(dp, rm).split('.');
  intp = arr[0];
  fracp = arr[1];
  intd = number.s < 0 ? intp.slice(1) : intp;
  nd = intd.length;
