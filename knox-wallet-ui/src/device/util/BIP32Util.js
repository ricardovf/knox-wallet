...
export default class BIP32Util {
  ...
  static splitPath(path) {
    if (path.length === 0) return Buffer.from([0x00]);

    let elements = path.split('/');
    if (elements.length > 10) throw new DeviceException('Path too long');
    ...
    for (let element of elements) {
      ...
      if (hardenedIndex > 0) {
        ...
        elementValue = elementValue.xor(bigInt(0x80000000));
      } else {
        elementValue = bigInt(element);
      }
      result.writeUInt32BE(elementValue, offset);
      offset += 4;
    }
